...
dimer = ade.Molecule(atoms=[
    ...
    ade.Atom("O", -0.01431300, 1.40222000, 0.00000000),
    ade.Atom("H", -0.17652200, 2.34993900, 0.00000000),
    ade.Atom("H", 0.94283200, 1.30476000, 0.00000000),
    ade.Atom("O", -0.01431300, -1.59566600, 0.00000000),
    ade.Atom("H", -0.26864800, -1.05356500, 0.75314700),
    ade.Atom("H", -0.26864800, -1.05356500, -0.75314700)
])
...
# Initial distance
r0 = dimer.atoms.distance(0, 5)

# range of distances
r_values = np.arange(1.4, 5, 0.2)
...
for r in r_values:
    ...
    filename = f"dimer7_translated_{r:.1f}.xyz"
...
xyz_files = glob.glob("dimer7_translated_*.xyz")
print(xyz_files)
...
def run_psi4(xyz_files):
    ...
    np.save('energy_values_dimer7.npy', energy_values)
    np.save('r_values_dimer7.npy', r_values)
