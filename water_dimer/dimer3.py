...
dimer = ade.Molecule(atoms=[
    ...
    ade.Atom("O", -0.06810181, -1.55133919, 0.00000000),
    ade.Atom("H", 0.80360137, -1.95391389, 0.00000000),
    ade.Atom("H", 0.08811991, -0.59656616, 0.00000000),
    ade.Atom("O", -0.01000397, 1.38574731, 0.00000000),
    ade.Atom("H", -0.89927694, 1.75082651, 0.00000000),
    ade.Atom("H", 0.58727473, 2.13837456, 0.00000000)
])
...
vec = dimer.atoms.nvector(2, 3)
unit_vec = vec / np.linalg.norm(vec)

# Initial distance
r0 = dimer.atoms.distance(2, 3)
...
for r in r_values:
    ...
    filename = f"dimer3_translated_{r:.1f}.xyz"
...
xyz_files = glob.glob("dimer3_translated_*.xyz")
print(xyz_files)
...
def run_psi4(xyz_files):
    ...
    for xyz_file in xyz_files:
        ...
        #calculate the vibrational frequencies at each distance
        frequencies = psi4.frequencies('scf/sto-3g', molecule=molecule)
        print(f"Frequencies for {xyz_file}: {frequencies}")
    ...
    np.save('energy_values_dimer3.npy', energy_values)
    np.save('r_values_dimer3.npy', r_values)
