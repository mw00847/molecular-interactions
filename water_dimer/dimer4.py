...
dimer = ade.Molecule(atoms=[
    ...
    ade.Atom("O", -0.05911000, 1.42419000, -0.04126700),
    ade.Atom("O", 0.05911000, -1.42419000, 0.04126700),
    ade.Atom("H", 0.57423300, 0.78111500, -0.38144600),
    ade.Atom("H", -0.57423300, -0.78111500, 0.38144600),
    ade.Atom("H", 0.46355400, 2.08814600, 0.41669200),
    ade.Atom("H", -0.46355400, -2.08814600, -0.41669200)
])

# Indices of atoms to be translated
idxs = (0, 2, 4)  # NOTE: atoms are 0 indexed

# Calculate the unit vector from O1 to O2 (index 0 to 3)
vec = dimer.atoms.nvector(3, 0)
unit_vec = vec / np.linalg.norm(vec)

# Initial distance
r0 = dimer.atoms.distance(3, 0)
...
for r in r_values:
    ...
    filename = f"dimer4_translated_{r:.1f}.xyz"
...
xyz_files = glob.glob("dimer4_translated_*.xyz")
print(xyz_files)
...
def run_psi4(xyz_files):
    ...
    np.save('energy_values_dimer4.npy', energy_values)
    np.save('r_values_dimer4.npy', r_values)
