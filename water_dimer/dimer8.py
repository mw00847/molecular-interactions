...
dimer = ade.Molecule(atoms=[
    ...
    ade.Atom("O", 0.00000000, 1.69425600, 0.00000000),
    ade.Atom("O", 0.00000000, -1.69425600, 0.00000000),
    ade.Atom("H", -0.50894500, 1.39478400, 0.75906300),
    ade.Atom("H", -0.50894500, 1.39478400, -0.75906300),
    ade.Atom("H", 0.50894500, -1.39478400, 0.75906300),
    ade.Atom("H", 0.50894500, -1.39478400, -0.75906300)
])
...
idxs = (2, 0, 3)  # NOTE: atoms are 0 indexed

# Calculate the unit vector from O1 to O2 (index 0 to 3)
vec = dimer.atoms.nvector(4, 0)
unit_vec = vec / np.linalg.norm(vec)

# Initial distance
r0 = dimer.atoms.distance(4, 0)
...
for r in r_values:
    ...
    filename = f"dimer8_translated_{r:.1f}.xyz"
...
xyz_files = glob.glob("dimer8_translated_*.xyz")
print(xyz_files)
...
def run_psi4(xyz_files):
    ...
    np.save('energy_values_dimer8.npy', energy_values)
    np.save('r_values_dimer8.npy', r_values)
