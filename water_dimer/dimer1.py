...
dimer = ade.Molecule(atoms=[
    ...
    ade.Atom("O", 0.00150700, 1.52308800, 0.00000000),
    ade.Atom("H", -0.89978500, 1.85460800, 0.00000000),
    ade.Atom("H", -0.07865400, 0.55730800, 0.00000000),

    ade.Atom("O", 0.00150700, -1.39265100, 0.00000000),
    ade.Atom("H", 0.47716300, -1.72770500, 0.76615900),
    ade.Atom("H", 0.47716300, -1.72770500, -0.76615900)
...
for r in r_values:
    ...
    filename = f"dimer1_translated_{r:.1f}.xyz"
...
xyz_files = glob.glob("dimer1_translated_*.xyz")
print(xyz_files)
...
def run_psi4(xyz_files):
    ...
    np.save('energy_values_dimer1.npy', energy_values)
    np.save('r_values_dimer1.npy', r_values)
