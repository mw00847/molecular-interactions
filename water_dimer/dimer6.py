...
dimer = ade.Molecule(atoms=[
    ...
    ade.Atom("O", 0.00000000, 1.40163100, 0.00000000),
    ade.Atom("O", 0.00000000, -1.40163100, 0.00000000),
    ade.Atom("H", 0.74728900, 0.79345600, 0.00000000),
    ade.Atom("H", -0.74728900, -0.79345600, 0.00000000),
    ade.Atom("H", 0.37598800, 2.28521000, 0.00000000),
    ade.Atom("H", -0.37598800, -2.28521000, 0.00000000)
])

# Indices of atoms to be translated
idxs = (1, 3, 5)  # NOTE: atoms are 0 indexed
...
for r in r_values:
    ...
    filename = f"dimer6_translated_{r:.1f}.xyz"
...
xyz_files = glob.glob("dimer6_translated_*.xyz")
print(xyz_files)
...
def run_psi4(xyz_files):
    ...
    np.save('energy_values_dimer6.npy', energy_values)
    np.save('r_values_dimer6.npy', r_values)
